...
export default function AdminDashboard() {
  ...
  // --- STATE FORM INPUT PRODUK (BAWAAN LU + TAMBAHAN STOK) ---
  const [name, setName] = useState("");
  const [price, setPrice] = useState("");
  const [category, setCategory] = useState("bold"); // Default ke kustomisasi kategori lu
  const [image, setImage] = useState("");
  const [description, setDescription] = useState("");
  const [tagline, setTagline] = useState("");
  const [origin, setOrigin] = useState("");
  const [notes, setNotes] = useState(""); 
  const [stock, setStock] = useState(10); // State stok baru sesuai request
  
  // State Matriks Rasa Bawaan Lu
  const [acidity, setAcidity] = useState(3);
  const [body, setBody] = useState(3);
  const [strength, setStrength] = useState(3);
  const [sweetness, setSweetness] = useState(3);

  // --- FUNGSI AMBIL ALL DATA (REALTIME SIMULATION) ---
  const fetchAllDashboardData = async () => {
    try {
      ...
      // 3. Ambil Data Log Login User (Sistem 1)
  ...
  // --- AKSES 1: SUNTIK PRODUK (SISTEM KATEGORI & STOK BARU) ---
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, "products"), {
        name,
        price: Number(price), 
        category, // Menampung kategori bold, medium, atau light
        image,
        description,
        tagline,
        origin,
        ...
        stock: Number(stock), // Request Sistem 3
        createdAt: new Date().toISOString(),
      });

      setMessage("✅ Mantap! Seduhan baru dengan kuota stok berhasil disuntik ke katalog Kopthree!");
  ...
  // --- AKSES 2: MANAJEMEN UPDATE STOK LANGSUNG DI TEMPAT ---
  const handleUpdateStock = async (id, newStock) => {
    try {
      await updateDoc(doc(db, "products", id), { stock: Number(newStock) });
      setMessage("📋 Stok berhasil diperbarui di server cloud.");
      fetchAllDashboardData();
    } catch (error) {
  ...
  // --- AKSES 3: HAPUS PRODUK BAWAAN LU ---
  ...
  // --- AKSES 4: BARISTA KENDALI STATUS KOPI ---
  const handleUpdateOrderStatus = async (orderId, currentStatus) => {
    const nextStatus = currentStatus === "SUCCESS" ? "DIPROSES" : "SELESAI";
    try {
      await updateDoc(doc(db, "orders", orderId), { status_pesanan: nextStatus });
      setMessage(`☕ Status order ${orderId} diubah menjadi ${nextStatus}`);
      fetchAllDashboardData();
    } catch (error) {
      setMessage(`❌ Gagal ubah status pesanan: ${error.message}`);
    }
  };

  // --- AKSES 5: RECOVERY EMAIL ACCOUNT USER (SISTEM 1) ---
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-6 sm:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="border-b border-neutral-900 pb-5 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          {/* MENU NAVIGASI INTERAKTIF ADMIN (3 SISTEM UTAMA) */}
        ...
        {/* ================= AREA KONTEN UTAMA (KONDISIONAL TABS) ================= */}
        
        {/* TAB 1: AREA SISTEM PESANAN (SISTEM REQUEST NO. 2) */}
        {activeTab === "orders" && (
                  ...
                  {orders.map((order) => (
                    <tr key={order.id} className="hover:bg-neutral-900/20 transition-colors">
                      ...
                      <td className="p-4 space-y-1">
                        ...
                        {order.orderType === "delivery" && <span className="block text-[11px] text-blue-400 font-medium">Ongkir Terhitung</span>}
                      ...
                      <td className="p-4">
                        <button onClick={() => handleUpdateOrderStatus(order.id, order.status_pesanan || "SUCCESS")} className="bg-neutral-900 hover:bg-amber-500 hover:text-neutral-950 border border-neutral-800 text-[10px] font-bold px-2.5 py-1.5 rounded-sm transition-all uppercase tracking-wide">
        ...
        {/* TAB 2: MANAGEMENT PRODUK (FORM GABUNGAN + FILTER RASA LU REQUEST NO. 3) */}
        {activeTab === "products" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Form Input (Bawaan Lu yang Dilengkapi Kategori Baru) */}
            <div className="bg-neutral-900 border border-neutral-800/60 p-6 rounded-sm h-fit space-y-4">
              ...
              <form onSubmit={handleAddProduct} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[9px] text-amber-500 uppercase tracking-widest font-bold mb-1">Kategori Rasa</label>
                    <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 p-2 text-xs rounded-sm text-amber-500 font-bold focus:outline-none focus:border-amber-500">
                      <option value="bold">Bold (Pekat)</option>
                      <option value="medium">Medium (Seimbang)</option>
                      <option value="light">Light (Halus)</option>
                    </select>
                  </div>
                  <div>
            ...
            {/* View Daftar Kopi & Kontrol Update Stok Langsung */}
                ...
                <table className="w-full text-left text-xs text-neutral-300">
                  <thead className="text-[10px] uppercase tracking-widest text-neutral-500 border-b border-neutral-800">
                    <tr>
                      ...
                      <th className="py-3 px-2">Kustomisasi Menu</th>
                  ...
                  <tbody className="divide-y divide-neutral-800/50">
                    {products.map((product) => (
                      <tr key={product.id} className="hover:bg-neutral-950/40 transition-colors">
                        ...
                        <td className="py-3 px-2 uppercase font-mono text-amber-500 text-[11px] font-bold">{product.category || "bold"}</td>
                        <td className="py-3 px-2 text-[10px] text-neutral-400 space-y-0.5">
                          <div className="text-neutral-500">• Kopi: <span className="text-white">Normal / Less Coffee</span></div>
                          <div className="text-neutral-500">• Gula: <span className="text-white">Normal / Less Sweet</span></div>
                        </td>
        ...
        {/* TAB 3: LOG LENGKAP PENGGUNA & RECOVERY (SISTEM REQUEST NO. 1) */}
        {activeTab === "users" && (
          <div className="space-y-4">
